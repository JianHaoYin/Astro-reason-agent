...
Usage:
    ...
    ./run_benchmark.py --benchmark revisit-optimization --case case_0001 --model anthropic::claude-sonnet-4-20250514
    
    # All cases, single model
    ./run_benchmark.py --benchmark stereo-imaging --all --model deepseek::deepseek-reasoner
    
    # All cases, multiple models
    ./run_benchmark.py --benchmark revisit-optimization --all --models anthropic::claude-sonnet-4 deepseek::deepseek-reasoner
    
    # With retry on invalid plans
    ./run_benchmark.py --benchmark stereo-imaging --all --model anthropic::claude-sonnet-4 --retry-on-invalid --max-retries 2
    
    # Baseline algorithm
    ./run_benchmark.py --benchmark revisit-optimization --case case_0001 --baseline greedy
    
    # Interactive mode
    ./run_benchmark.py --benchmark regional-coverage --case case_0001 --interactive
    
    # From experiment config file
    ./run_benchmark.py --experiment-config experiments/my_experiment.json
...
def setup_sandbox(benchmark: str, case_id: str, output_dir: Path) -> Path:
    ...
    case_data_dir = BENCHMARK_DATA_DIR / benchmark_slug / "cases" / case_id
...
def run_baseline(benchmark: str, case_id: str, algorithm: str, output_dir: Path) -> int:
    ...
    benchmark_slug = benchmark.replace("-", "_")
    baseline_script = PROJECT_ROOT / "src" / "benchmark" / "scenarios" / benchmark_slug / "baselines" / f"{algorithm}.py"
...
def run_experiment_batch(
    ...
    benchmark_slug = benchmark.replace("-", "_")
    case_dir = BENCHMARK_DATA_DIR / benchmark_slug / "cases"
...
Examples:
  ...
  ./run_benchmark.py --benchmark latency-optimization --case case_0001 --bwrap --memory-limit 4G --cpu-quota 200%%
  
  # Legacy format (still works)
  ./run_benchmark.py --benchmark revisit-optimization --case case_0001 --model claude-sonnet-4-20250514
        """
    )
